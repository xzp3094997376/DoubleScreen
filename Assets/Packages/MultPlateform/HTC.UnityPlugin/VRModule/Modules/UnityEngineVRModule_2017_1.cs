...
namespace HTC.UnityPlugin.VRModuleManagement
{
    public sealed partial class UnityEngineVRModule : VRModule.ModuleBase
    {
        ...
        public override void BeforeRenderUpdate()
        {
            ...
            //Debug.LogError("Test   "+ currentPlatform);
            if (currentPlatform != PlatformType.zSpace && currentPlatform != PlatformType.PC)
            {
            ...
            {
                m_nodeStateList = new List<XRNodeState>() { new XRNodeState()
                                                                            { uniqueID = 104246ul,
            ...
            for (int i = 0, imax = m_nodeStateList.Count; i < imax; ++i)
            {
                ...
                //if (!prevState.isConnected)
                {
                    ...
                    {
                        if (currentPlatform == PlatformType.PC || currentPlatform == PlatformType.zSpace)
                        {
                            currState.serialNumber = currentPlatform + " " + m_nodeStateList[i].uniqueID.ToString("X8");
                            currState.modelNumber = currentPlatform + " " + m_nodeStateList[i].nodeType;
                            currState.renderModelName = currentPlatform + " " + m_nodeStateList[i].nodeType;
                        }
                        else
                        {
            ...
            //Debug.LogError(leftIndex + "     " + rightIndex);
