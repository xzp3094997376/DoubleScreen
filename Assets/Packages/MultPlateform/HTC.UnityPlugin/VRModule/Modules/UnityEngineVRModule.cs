...
namespace HTC.UnityPlugin.VRModuleManagement
{
    public sealed partial class UnityEngineVRModule : VRModule.ModuleBase
    {
        ...
        {
            PlatformType current = (PlatformType)Enum.Parse(typeof(PlatformType), PlayerPrefs.GetString("CurrentPlateform", "PC"));
            return true;//打包后这里为true，编辑器下为false
            if (Application.isEditor
                && current == PlatformType.zSpace)
                return true;
            else if (!Application.isEditor
                && (current == PlatformType.PC))
                return true;
            else if (!Application.isEditor
                && (current == PlatformType.zSpace))
                return true;
            else
                return VIUSettings.activateUnityNativeVRModule && XRSettings.enabled;
        }
