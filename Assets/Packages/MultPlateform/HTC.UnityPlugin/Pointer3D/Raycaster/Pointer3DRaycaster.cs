...
namespace HTC.UnityPlugin.Pointer3D
{
    ...
    public class Pointer3DRaycaster : BaseFallbackCamRaycaster
    {
        ...
        public delegate void RaycasterObject(GameObject go, bool bol);
        public RaycasterObject raycasterHover;
        private RaycastResult _raycast;
        /// <summary>
        /// 射线检测到的对象
        /// </summary>
        public RaycastResult _Result
        {
            get
            {
                return _raycast;
            ...
            set
            {
                if (value.gameObject == null && _raycast.gameObject != null)
                {
                    //Debug.Log("离开RaycastResult:" + _raycast.gameObject.name);
                    //离开

                    //raycasterHover?.Invoke(_raycast.gameObject, false);

                    if (raycasterHover != null)
                    {
                        raycasterHover.Invoke(_raycast.gameObject, false);

                    }
                }
                else if (value.gameObject != null && _raycast.gameObject == null)
                {
                    //进入
                    //Debug.Log("进入RaycastResult:" + value.gameObject.name  );
                    //raycasterHover?.Invoke(value.gameObject, true);
