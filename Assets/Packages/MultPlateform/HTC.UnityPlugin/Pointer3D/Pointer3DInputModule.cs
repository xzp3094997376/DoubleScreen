...
namespace HTC.UnityPlugin.Pointer3D
{
    public class Pointer3DInputModule : BaseInputModule
    {
        ...
        public void SetCoexistValue(bool bol)
        {
            coexist = bol;
            StandaloneInputModule sli = this.gameObject.GetComponent<StandaloneInputModule>();

            if (!bol)
            {
                if (sli != null)
                {
                    DestroyImmediate(sli);
                }
        ...
        public DisplayIndex CurrentDisplay=DisplayIndex.Display1;//zxj
        protected virtual void Update()
        {
            //if (MultiDisplayUtil.GetCurrentDisplay() != CurrentDisplay)//zxj ÔÝÊ±×¢ÊÍ
            //{
            //    return;
            //}
        ...
        protected virtual void ProcessRaycast()
        {
            ...
            for (int i = 0, imax = raycasters.Count; i < imax; ++i)
            {
                ...
                if (r != null)
                {
                    if (!processingRaycasters.Contains(r))
                    {
